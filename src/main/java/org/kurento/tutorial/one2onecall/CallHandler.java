...
import org.kurento.tutorial.one2onecall.auth.Credential;
import org.kurento.tutorial.one2onecall.auth.UserProxy;
...
public class CallHandler extends TextWebSocketHandler {
  ...
  private void register(WebSocketSession session, JsonObject jsonMessage) throws IOException {
    ...
    if (name.isEmpty()) {
      ...
      credential.setPassword("iTech1234");
      ResponseEntity<APIResponse<Object>> responseResponseEntity = userProxy.login(credential);
      if (responseResponseEntity.getStatusCode().is2xxSuccessful() && responseResponseEntity.getBody() != null) {
        registry.register(caller);
      }
      else responseMsg = "rejected: user '" + name + "' is not exist";
